...
public class RestTemplateLogic {
	 ...
	 * 
	 * @param builder      URLビルダー
	 * @param genericyType レスポンスクラス
	 * 
	 * @return HttpResponse
	 */
	...
	/**
	 * HttpRequest Post 引数のビルダーとレスポンスクラスを元にHttpRequest(POST)を行う。
	 * 
	 * @param builder      URLビルダー
	 * @param genericyType レスポンスクラス
	 * 
	 * @return HttpResponse
	 */
	public <T> T post(String endpoint, BaseReqModel reqModel, Class<T> genericyType) {

		// ヘッダー設定
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Encoding", "UTF-8");
		headers.add("Content-Type", "application/json; charset=UTF-8");
		HttpEntity<?> requestEntity = new HttpEntity<>(reqModel, headers);

		// コンソール出力
		System.out.println("実行するリクエスト:" + endpoint);

		// HttpRequest実行
		ResponseEntity<T> response;
		try {
			response = restTemplate.postForEntity(endpoint, requestEntity, genericyType);
		} catch (Exception e) {
			// コンソール出力
			System.err.println("えらー");
			// stackTraceをコンソールに出力
